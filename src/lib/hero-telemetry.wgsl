...
fn connectorMark(p: vec2f) -> f32 {
  let q = rotate(p, -0.08);
  var mark = sdRoundedBox(q, vec2f(0.16, 0.22), 0.02);
  let rack = sdRoundedBox(q - vec2f(0.22, 0.0), vec2f(0.05, 0.18), 0.01);
  mark = opSmoothUnion(mark, rack, 0.02);
  for (var i = 0; i < 4; i = i + 1) {
    let fi = f32(i);
    let pin = sdEllipse(q - vec2f(-0.06 + fi * 0.055, 0.16), vec2f(0.012, 0.03));
    mark = min(mark, pin);
  }
  let fiber = sdEllipse(rotate(q - vec2f(0.02, -0.08), 0.6), vec2f(0.16, 0.018));
  mark = opSmoothUnion(mark, fiber, 0.025);
  return mark;
}
...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  let navy = vec3f(0.102, 0.227, 0.322);
  ...
  a *= 0.55 * leftClear;
  a = clamp(a, 0.0, 0.34);
  let col = mix(navy, paper, spark * 0.45 + line * 0.05);
